...
- `Tip` :  Mac OS 기준으로 `cmd + n`을 누르면 generate가 나오는데 거기서 getter and setter를 선택하면 `getName()`과 `setName()` 함수를 입력할 수 있다. 이것을 JavaBean 규약이라고 하며, property 접근 방식이라고도 한다.
...
- 데이터 저장소가 선정되지 않아서, 먼저 인터페이스로 구현 클래스를 변경할 수 있게 설계
- 
